...
destroy('/hooks/:id', function() {
  var express = this

  hooks.destroy(this.param('id'), function(err) {
    express.removeHook(express.param('id'), function() {
      ircBot.emit('partChannel', {channel: express.param('channel')})
      express.flash('message', 'Removed the webhook successfully!')
      express.redirect("/hooks")
    })
  })
})
